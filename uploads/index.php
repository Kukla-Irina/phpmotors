...
switch ($action) {
    case 'upload':
        ...
        if ($imageCheck) {
            $message = '<p class="notification">An image by that name already exists.</p>';
        } elseif (empty($invId) || empty($imgName)) {
            $message = '<p class="notification">You must select a vehicle and image file for the vehicle.</p>';
        } else {
            ...
            if ($result) {
                $message = '<p class="notification">The upload succeeded.</p>';
            } else {
                $message = '<p class="notification">Sorry, the upload failed.</p>';
            }
        }
    ...
    case 'delete':
        ...
        if ($remove) {
            $message = "<p class='notification'>$filename was successfully deleted.</p>";
        } else {
            $message = "<p class='notification'>$filename was NOT deleted.</p>";
        }
    ...
    default:
        ...
        if (count($imageArray)) {
            ...
            $imageDisplay = '<p class="notification">Sorry, no images could be found.</p>';
        }
