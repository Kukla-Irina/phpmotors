...
switch ($action){
    ...
    case 'addReview':
        ...
        if($AddReviewReport === 1){
            $_SESSION['message'] = '<p class="notification">Review has been successfully added.</p>';
            header('Location: ' . $_SERVER['HTTP_REFERER']);
            exit;
        } else {
            $_SESSION['message'] = '<p class="notification">Sorry, an error occurred. Please try again.</p>';
            header('Location: ' . $_SERVER['HTTP_REFERER']);
            exit;
        }
        break;
    ...
    case 'editReview':
        ...
        if ($updateReport == 1){
            $_SESSION['message'] = '<p class="notification">The post was successfully updated.</p>';
        } else {
            $_SESSION['message'] = '<p class="notification">Sorry, an error occurred. Please try again.</p> '.$deleteReport;
        }
    ...
    case 'deleteReview':
        ...
        // Generate the correct message.
        if ($deleteReport == 1){
            $_SESSION['message'] = '<p class="notification">The post was successfully deleted.</p>';
        } else {
            $_SESSION['message'] = '<p class="notification">Sorry, an error occurred. Please try again.</p> '.$deleteReport;
        }
